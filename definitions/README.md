...
└── definitions/                  # Dataform パイプライン定義ディレクトリ
    ...
    │   ├── assert_google_cloud_certifications_match_scraping.sqlx
...
- **`assert_google_cloud_certifications_match_scraping` (追加)**: `google_cloud_certifications` テーブル（資格マスタ）の内容が、Playwright のスクレイピングにより取得した公式サイトの最新資格情報（資格名、レベル、有効期間）と一致しているか検証するアサーションです。一時テーブルを作成せず、SQL 内から直接スクレイピングリモート関数を呼び出して突合します。
