...
| `scrape-gcp-certifications` | `scrape-gcp-certifications/` | HTTP (BigQuery Remote Function) | Google Cloud 認定資格の公式サイトをスクレイピングし、資格一覧を返却 |
...
## 4. scrape-gcp-certifications

BigQuery リモート関数から HTTP リクエストを介して呼び出され、Google Cloud 認定資格の公式サイトから最新の資格一覧（資格名・レベル・有効期間）を Playwright でスクレイピングして取得します。

1. Playwright (Chromium ヘッドレスブラウザ) をコンテナ上で起動
2. Google Cloud 認定資格の公式サイトをスクレイピング
3. リモート関数からのバッチリクエストを受理し、JSON 配列文字列として BigQuery へ返却

詳細は [`scrape-gcp-certifications/README.md`](scrape-gcp-certifications/README.md) を参照してください。
